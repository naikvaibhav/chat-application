...
@Component({
  ...
  //socket service is local to the component
...
export class ChatboxComponent implements OnInit {
  ...
  public messageList : any = [];// stores current message list display in chat box
  ...
  public sendMessage : any=()=>{
    if(this.messageText){
      ...
      this.toastr.warning('text message cant be empty')
    }
  }// end sendMessage

  //pushing the chat to window and clearing the chat box
  ...
  //for receiving a chat message from a user
  ...
  //selecting the receiver to chat with
 public userSelectedToChat: any = (id,name)=>{
   ...
   this.receiverId = id;
   //empty the message list as we switch to a new receiver
   this.messageList = [];
   this.pageValue = 0;
 ...
 //use to paginate the previous message so that the server doesnt crash
 public getPreviousChatWithAUser : any =()=>{
   ...
   this.SocketService.getChat(this.userInfo.userId, this.receiverId, this.pageValue * 10).subscribe((apiResponse)=>{
     ...
     //if success load previous messages
